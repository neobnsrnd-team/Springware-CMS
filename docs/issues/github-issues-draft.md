...
**작업 내용**
- [x] 플러그인 CSS (`app-header/style.css`)에서 필요한 스타일을 추출해 HTML 인라인 스타일로 이식
- [x] `data-cb-type="app-header"` 속성 제거
- [x] mobile / web / responsive 3개 variant 모두 변환
- [x] DB `DATA` 필드의 `html` 값 업데이트 (`scripts/migrate-app-header-to-html.ts`)
- [x] 에디터에서 더블클릭으로 은행명·로고 등 텍스트·이미지 직접 편집 가능한지 확인
- [x] 뷰어(`/view`)에서 렌더링 정상 확인
- [x] **[retrofit — Issue #10]** 3개 variant HTML 루트 요소에 `data-component-id="app-header-{mobile|web|responsive}"` 추가 및 마이그레이션 스크립트 재실행
...
- [x] `data-cb-type="product-menu"` 속성 제거
- [x] 루트 요소에 `data-component-id="product-menu-{mobile|web|responsive}"` 추가
- [x] mobile / web / responsive 3개 variant 변환 (`scripts/migrate-product-menu-to-html.ts`)
- [x] DB `DATA` 필드 `html` 값 업데이트 (마이그레이션 스크립트 실행 완료)
- [x] 에디터에서 더블클릭으로 메뉴 텍스트 직접 편집 가능 확인
- [x] 뷰어 렌더링 정상 확인
- [x] **[추가 기능]** SVG 아이콘 교체·색상 변경 커스텀 편집 모달 구현 (`ProductMenuIconEditor.tsx`)
  - 아이콘 항목 클릭 시 ContentBuilder `#divLinkTool`에 편집 버튼 자동 주입
  - 12종 프리셋 SVG 아이콘 그리드, 이미지 업로드, 개별/전체 색상 변경
...
- [ ] 루트 요소에 `data-component-id="auth-center-{mobile|web|responsive}"` 추가
- [ ] mobile / web / responsive 3개 variant 변환
- [ ] DB `DATA` 필드 `html` 값 업데이트 (`scripts/` 마이그레이션 스크립트)
- [ ] 에디터에서 더블클릭으로 카드 제목·설명 텍스트 직접 편집 가능한지 확인
- [ ] 뷰어 렌더링 정상 확인
...
- [ ] 루트 요소에 `data-component-id="site-footer-{mobile|web|responsive}"` 추가
- [ ] mobile / web / responsive 3개 variant 변환
- [ ] DB `DATA` 필드 `html` 값 업데이트 (`scripts/` 마이그레이션 스크립트)
...
- [ ] 루트 요소에 `data-component-id="media-video-{mobile|web|responsive}"` 추가
- [ ] YouTube iframe을 표준 HTML `<iframe>` 태그로 유지
- [ ] mobile / web / responsive 3개 variant 변환
- [ ] DB `DATA` 필드 `html` 값 업데이트 (`scripts/` 마이그레이션 스크립트)
- [ ] 에디터에서 더블클릭으로 제목 텍스트 직접 편집 가능한지 확인
- [ ] 뷰어 렌더링 정상 확인
...
승인 시점에 `SPW_CMS_PAGE_COMP_MAP`에 삽입할 컴포넌트 목록을 파악하는 방법이 없어진다.
...
[Issue #1~#7 이후 — 식별자 소실]
페이지 HTML: (속성 없음)  →  DB 연결 불가 → PAGE_COMP_MAP 추적 불가
...
- [ ] Issue #1 retrofit — `app-header` 3개 variant HTML에 `data-component-id` 추가 및 마이그레이션 스크립트(`scripts/migrate-app-header-to-html.ts`) 재실행
- [ ] Issue #2~#7 — 각 컴포넌트 변환 시 `data-component-id` 포함을 필수 요건으로 적용 (각 이슈와 병행)
- [ ] `docs/guide/플러그인-컴포넌트-HTML-변환-가이드.md` — `data-component-id` 요건 추가
- [ ] 승인 시 `SPW_CMS_PAGE_COMP_MAP` insert 로직에서 `data-component-id`를 파싱해 컴포넌트 목록 추출 (별도 이슈 또는 본 이슈에서 함께 처리)
...
- 승인 API (PAGE_COMP_MAP insert 로직)
