...
## Issue #1 — `app-header` 컴포넌트 순수 HTML 변환 ✅ (retrofit 필요)
...
- [ ] **[retrofit — Issue #10]** 3개 variant HTML 루트 요소에 `data-component-id="app-header-{mobile|web|responsive}"` 추가 및 마이그레이션 스크립트 재실행
...
- [ ] 루트 요소에 `data-component-id="product-menu-{mobile|web|responsive}"` 추가
- [ ] mobile / web / responsive 3개 variant 변환
- [ ] DB `DATA` 필드 `html` 값 업데이트 (`scripts/` 마이그레이션 스크립트)
- [ ] 에디터에서 더블클릭으로 메뉴 텍스트·아이콘 직접 편집 가능한지 확인
- [ ] 뷰어 렌더링 정상 확인
