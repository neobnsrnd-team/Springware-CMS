-- ============================================================================
-- Springware CMS — Oracle 19c DDL 통합 스크립트
-- ============================================================================
...
--       - ORA-02289: 시퀀스 없음 (DROP SEQUENCE)
...
DROP SEQUENCE SPW_COMP_MAP_SEQ;


-- ════════════════════════════════════════════════════════════════════════════
-- [SECTION 1] 시퀀스 — COMP_PAGE_MAP SEQ PK 자동증가용
-- ════════════════════════════════════════════════════════════════════════════

CREATE SEQUENCE SPW_COMP_MAP_SEQ
    START WITH 1
    INCREMENT BY 1
    NOCACHE
    NOCYCLE;
...
-- ──────────────────────────────────────────────────────────────────────────
-- 2-1. SPW_CMS_COMPONENT — 컴포넌트 카탈로그 (금융 + 기본 블록)
-- ──────────────────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────────────────
-- 3-1. SPW_CMS_PAGE_HISTORY — 페이지 버전 이력
-- ──────────────────────────────────────────────────────────────────────────
CREATE TABLE SPW_CMS_PAGE_HISTORY (
    ...
    FINAL_APPROVAL_USER_ID   VARCHAR2(100),
    FINAL_APPROVAL_USER_NAME VARCHAR2(200),
    SNAPSHOT_DTIME           TIMESTAMP      DEFAULT SYSTIMESTAMP,
    RENDERED_HTML            CLOB,
...
COMMENT ON TABLE  SPW_CMS_PAGE_HISTORY                     IS 'CMS 페이지 변경 이력 — 결재 상태 변경 시마다 스냅샷 [Springware CMS 신규]';
COMMENT ON COLUMN SPW_CMS_PAGE_HISTORY.VERSION             IS '이력 버전 번호 (상태 변경 시마다 +1)';
COMMENT ON COLUMN SPW_CMS_PAGE_HISTORY.SNAPSHOT_DTIME      IS '스냅샷 생성 일시';
COMMENT ON COLUMN SPW_CMS_PAGE_HISTORY.RENDERED_HTML       IS '롤백용 렌더링 HTML 전문 (CLOB) — FILE_PATH 덮어쓰기로 즉시 복원';


-- ──────────────────────────────────────────────────────────────────────────
-- 3-2. SPW_CMS_COMP_PAGE_MAP — 페이지-컴포넌트 매핑
--      SEQ: IDENTITY 대신 SEQUENCE + BEFORE INSERT 트리거 방식 (DBeaver 호환)
-- ──────────────────────────────────────────────────────────────────────────
CREATE TABLE SPW_CMS_COMP_PAGE_MAP (
    ...
    LAST_MODIFIER_ID    VARCHAR2(100),
    LAST_MODIFIED_DTIME TIMESTAMP      DEFAULT SYSTIMESTAMP,
    CONSTRAINT PK_SPW_CMS_COMP_PAGE_MAP PRIMARY KEY (SEQ),
    CONSTRAINT FK_SPW_COMP_MAP_PAGE     FOREIGN KEY (PAGE_ID)
        REFERENCES SPW_CMS_PAGE (PAGE_ID),
    CONSTRAINT FK_SPW_COMP_MAP_COMP     FOREIGN KEY (COMPONENT_ID)
        REFERENCES SPW_CMS_COMPONENT (COMPONENT_ID)
);

COMMENT ON TABLE  SPW_CMS_COMP_PAGE_MAP                    IS '페이지-컴포넌트 매핑 — 동일 컴포넌트 다중 배치 허용 [Springware CMS 신규]';
COMMENT ON COLUMN SPW_CMS_COMP_PAGE_MAP.SEQ                IS '시퀀스 자동증가 PK (SPW_COMP_MAP_SEQ)';
...
CREATE INDEX IDX_SPW_COMP_MAP_PAGE
    ON SPW_CMS_COMP_PAGE_MAP (PAGE_ID, SORT_ORDER);
...
WHERE sequence_name = 'SPW_COMP_MAP_SEQ';
